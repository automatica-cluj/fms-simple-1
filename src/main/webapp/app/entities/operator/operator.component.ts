...
export class OperatorComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.operatorService
      ...
      .subscribe(
        (res: HttpResponse<IOperator[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInOperators(): void {
    this.eventSubscriber = this.eventManager.subscribe('operatorListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IOperator[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
