...
describe('OperatorWorkShift e2e test', () => {
  ...
  /* it('should create and save OperatorWorkShifts', async () => {
        ...
        await promise.all([
            ...
            operatorWorkShiftUpdatePage.statusSelectLastOption(),
            operatorWorkShiftUpdatePage.deviceSelectLastOption(),
            operatorWorkShiftUpdatePage.operatorSelectLastOption(),
        ]);

        expect(await operatorWorkShiftUpdatePage.getStartDateInput()).to.contain('2001-01-01T02:30', 'Expected startDate value to be equals to 2000-12-31');
        expect(await operatorWorkShiftUpdatePage.getEndDateInput()).to.contain('2001-01-01T02:30', 'Expected endDate value to be equals to 2000-12-31');
        ...
        expect(await operatorWorkShiftComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1, 'Expected one more entry in the table');
    }); */
  ...
  /* it('should delete last OperatorWorkShift', async () => {
        ...
        expect(await operatorWorkShiftDeleteDialog.getDialogTitle())
            .to.eq('ifmSimple1App.operatorWorkShift.delete.question');
